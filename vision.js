 ...
 *
 * 配置（三者优先级从高到低）:
 *   - 环境变量: DASHSCOPE_API_KEY / VISION_MODEL / DASHSCOPE_BASE_URL
 *   - 同目录 .env 文件（本项目 .env 已被 .gitignore 忽略）
 */
...
// 手动解析 .env（无依赖，避免在项目根引入 node_modules）
function loadDotenv() {
  const candidates = [
...
async function main() {
  ...
  if (!MODEL) {
    console.error("请设置 VISION_MODEL 环境变量或在 .env 文件中配置（如 qwen-vl-max）。");
    process.exit(1);
  }
